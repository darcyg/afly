...
int main(int argc, char *argv[])
{
    if (write_pid() != 0)
    {
        LOG("Warning: Amber Program Has Started!");
        return -1;
    }

    INIT_LOG("nxp");

    LOG("ATTR_LOCK_DEL_FP_TEMP is [%s].\n", ATTR_LOCK_DEL_FP_TEMP);

    system("mkdir -p /etc/config/dusun/nxp");
    write_version("/etc/config/dusun/nxp/version");
    ...
    get_mac_address(gw_mac);
    proto_init(MOD_NXP, attribute_handlers, NULL, NULL);
